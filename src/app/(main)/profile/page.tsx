...
import { User, MapPin, Lock, LogOut, Mail, Phone, Calendar, Edit2, Save, Eye, EyeOff, FileX } from 'lucide-react';
...
// Component Sidebar
const Sidebar: React.FC<SidebarProps> = ({ activePage, setActivePage, userInfo, onLogout }) => {
  ...
  const subname = `${a[0][0]?.toLocaleUpperCase()}${a[a.length -1][0]?.toLocaleUpperCase()}`
  console.log(subname)
function getRandomColor() {
  ...
  return (
    ...
    background: `linear-gradient(to bottom right, ${bgfrom}, ${bgto})`
          ...
          <h3 className="mt-4 text-lg font-semibold text-gray-800">{userInfo.name}</h3>
...
// Component Chi tiết
const ProfileDetails: React.FC<ProfileDetailsProps> = ({ userId,userInfo, setUserInfo }) => {
...
const MyTours = ( {tourorder}:MyToursProps) => {
      ...
      return (
        <div className='fixed inset-0 z-20 bg-black/50 flex justify-center items-center '>
          <div className='p-8 bg-white flex flex-col gap-4 border rounded-2xl w-[800px]'>
            <button className='mb-4' onClick={()=>setIsopen(false)}> <X /></button>
            {list?.items.map((item,idx) =><Subitem touritem={item} key={idx}/>  )}
            </div>
        </div>
      )
    }
    ...
    <div className="bg-white rounded-xl shadow-lg p-8 relative">
      ...
      <div className="space-y-4">
        {tourorder?.map((tour, idx) => (
          <div onClick={()=>{setIdorder(tour.id), setIsopen(true)}} key={idx} className="border border-gray-200 rounded-lg p-4 hover:shadow-md transition-shadow">
            <div className="flex justify-between items-start">
              <div>
                <h3 className="font-semibold text-gray-800 mb-1">Đơn {idx+1}</h3>
                <p className="text-sm text-gray-600">Ngày đi: {format(tour.created_at)} </p>
              </div>
              <span className={`px-3 py-1 rounded-full text-sm font-medium ${
                tour.paymentMethodName === 'VNPay' ? 'bg-green-100 text-green-700' :
                'bg-yellow-100 text-yellow-700'
              }`}>
                {tour.paymentMethodName === 'VNPay' ? 'Đã thanh toán' : 'Chưa thanh toán'}
              </span>
            </div>
          </div>
        ))}
...
const ChangePassword = ({ userId }: ChangePasswordProps) => {
  ...
  const handleChangePassword = async(): Promise<void> => {
    ...
    try{
     const res = await fetch(`http://localhost:8088/api/users/change-password/${userId}`,{
        method:"PUT",
         headers: {
        "Content-Type": "application/json",
      },
...
const ProfilePage: React.FC = () => {
  ...
  const renderContent = () => {
    switch (activePage) {
      case 'details':
        return <ProfileDetails userId={userId} userInfo={userInfo} setUserInfo={setUserInfo} />;
      case 'tours':
        return <MyTours tourorder={tourorder} />;
      case 'password':
